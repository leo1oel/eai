...
def q(cfg, act_fn=nn.ELU()):
	...
	return nn.Sequential(nn.Linear(cfg.obs_dim+cfg.action_dim, cfg.mlp_dim), nn.LayerNorm(cfg.mlp_dim), nn.Tanh(),
...
class ReplayBuffer():
	...
	def __len__(self):
		return self.capacity if self._full else self.idx
